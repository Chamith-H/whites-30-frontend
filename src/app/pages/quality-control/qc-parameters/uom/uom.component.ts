...
export class UomComponent {
  ...
  create() {
    this.modalRef = this.modalService.show(UomFormComponent, {
      initialState: {
  ...
  getData(page: number, filter: any) {
    ...
    this.qcParameterService.getAllUom(page, filter).subscribe({
      next: (res) => {
  ...
  async do_TableAction(option: TAction) {
    if (option.action === "delete") {
      this.isLoading = true;
      this.qcParameterService.deleteUom(option.id).subscribe({
        next: (data: sMsg) => {
